...
#define DEVICE_NAME                "HL600_tester"                             /**< Name of device. Will be included in the advertising data. */
...
const app_uart_comm_params_t comm_params_communicate_with_nrf51822 =
       {
           UART_CC2640_RX, //RX_PIN_NUMBER,
           UART_CC2640_TX, //TX_PIN_NUMBER,
           RTS_PIN_NUMBER,
           CTS_PIN_NUMBER,
           APP_UART_FLOW_CONTROL_DISABLED,
           false,
...
const app_uart_comm_params_t comm_params_monitor_nrf51822 =
       {
           UART_GPRS_TX, //RX_PIN_NUMBER,
           UART_GPRS_RX, //TX_PIN_NUMBER,
           RTS_PIN_NUMBER,
           CTS_PIN_NUMBER,
           APP_UART_FLOW_CONTROL_DISABLED,
           false,
...
const app_uart_comm_params_t comm_params_communicate_with_sim868 =
       {
           UART_GPRS_RX, //RX_PIN_NUMBER,
           UART_GPRS_TX, //TX_PIN_NUMBER,
           RTS_PIN_NUMBER,
           CTS_PIN_NUMBER,
           APP_UART_FLOW_CONTROL_DISABLED,
           false,
...
void uart_communicate_pin_set_input(void)
{
  ...
  nrf_gpio_cfg_input(UART_CC2640_TX, NRF_GPIO_PIN_PULLUP);
  nrf_gpio_cfg_input(UART_CC2640_RX, NRF_GPIO_PIN_PULLUP);
  nrf_gpio_cfg_input(UART_GPRS_TX, NRF_GPIO_PIN_PULLUP);
  nrf_gpio_cfg_input(UART_GPRS_RX, NRF_GPIO_PIN_PULLUP);
  #else
  nrf_gpio_cfg_input(UART_PC_TX, NRF_GPIO_PIN_PULLUP);
  nrf_gpio_cfg_input(UART_PC_RX, NRF_GPIO_PIN_NOPULL);
  nrf_gpio_cfg_input(UART_CC2640_TX, NRF_GPIO_PIN_PULLUP);
  nrf_gpio_cfg_input(UART_CC2640_RX, NRF_GPIO_PIN_NOPULL);
  nrf_gpio_cfg_input(UART_GPRS_TX, NRF_GPIO_PIN_PULLUP);
  nrf_gpio_cfg_input(UART_GPRS_RX, NRF_GPIO_PIN_NOPULL);
  #endif
}
    ...
    for (;;)
    {
      ...
      if(Flag.is_uart_sw_to_pc)
      {
        ...
        if(Flag.is_return_result_to_pc)
        {
          ...
          }while(communicate_time_cnt<=(100/CHECK_TIME_TIMEOUT_INTERVAL));
      ...
      /****************************///切换到监控天王星板子上nrf51822发送给sim868的数据
